...
async def test_build_and_deploy(ops_test, charm_under_test, tls_enabled):
    ...
    # THEN no app goes into error state and the model empty
    # TODO when the app removal Juju bug is fixed, replace the following with a wait_for_idle with
    #  raise_on_error=True and assert len(ops_test.model.applications) == 0

    # Sometimes it take time to remove an app, and sometimes juju never really finishes removing
    # Sleep for a bit and then force removal.
    ...
    # Note: Removing all apps is also needed to clean the model for the next parametrization item.
